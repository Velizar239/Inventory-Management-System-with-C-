...
// Проверка за съществуващо ID
int idExists(int id) {
    FILE *fp = fopen(FILE_NAME, "r");
    if (fp == NULL) return 0; 

    struct Item temp;
    while (fscanf(fp, "%d %s %s %f %s", &temp.id, temp.name, temp.type, &temp.quantity, temp.unit) != EOF) {
        if (temp.id == id) {
            fclose(fp);
            return 1; // ID съществува
        }
    }

    fclose(fp);
    return 0; // ID не е намерено
}
...
void addItem() {
    ...
    scanf("%d", &item.id);
    if (idExists(item.id)) {
        printf("This ID already exists! Enter a different ID.\n");
    }
    } while (idExists(item.id));
    ...
    // Проверка за положително число
    do {
    printf("Enter Quantity: ");
    if (scanf("%f", &item.quantity) != 1) {
